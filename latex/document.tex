...
\item sensor board includes a three-axis accelerometer, temperature sensor, light sensor, 8 tricolor LEDs, analogue and digital inputs, two momentary switches, and 4 high current output pins.
...
\begin{figure}[!ht]
	\caption{Movement sensor}
	\label{fig:movement_sensor_component}
	\vspace{-1em}
	\centering
	\includegraphics[width=0.78\textwidth]{{"./img/Drawio/Components/Normal sensor"}.png}
\end{figure}
\begin{figure}[!ht]
	\caption{Light sensor}
	\label{fig:light_sensor_component}
	\vspace{-1em}
	\centering
	\includegraphics[width=0.78\textwidth]{{"./img/Drawio/Components/Light Sensor"}.png}
\end{figure}
\begin{figure}[!ht]
...
\begin{figure}[!ht]
	\caption{Movement sensor}
	\label{fig:movement_sensor_class}
	\vspace{-1em}
	\centering
...
\chapter{Testing}
\chapter{Conclusion}
In this project we created a smart lighting system which reacts dynamically on movement of vehicles or pedestrians and is also managed remotely. Operator can turn the lights on or off, check if the sensor works, get current light intensity, change the threshold for night and day mode and also get the statistical data for use analysis.  

The system could be vastly improved by better sensors which would not have to be wireless. The system might either be connected to metro-optical network or use power-line communication. 
\printglossaries
